...
with c1:
    f_stock = st.file_uploader("1️⃣ Stock Actual", type=["xlsx", "xls"], key="stock_file")
with c2:
    f_sal = st.file_uploader("2️⃣ Salidas Históricas", type=["xlsx", "xls"], key="sal_file")
with c3:
    f_oc = st.file_uploader("3️⃣ Órdenes de Compra", type=["xlsx", "xls"], key="oc_file")
...
if result is not None:
    ...
    oc = st.session_state.get("oc_data", pd.DataFrame())
    issues = st.session_state.get("issues_data", [])
