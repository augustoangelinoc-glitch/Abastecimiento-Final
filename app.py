...
with st.sidebar:
    ...
    only_zero_consumo=st.checkbox("Stock cero con salidas")
...
if only_zero_consumo:view=view[(view.stock_actual<=0)&(view.consumo_total>0)]
...
with tabs[0]:
    rot=df[df.situacion_stock=="ROTURA DE STOCK"]
    zero=df[(df.stock_actual<=0)&(df.consumo_total>0)]
    if len(rot)>0:
        st.error(f"🔴 ATENCIÓN: {len(rot):,} materiales presentan ROTURA DE STOCK. Tienen salidas/consumo y actualmente no tienen stock.")
        st.dataframe(
            rot[["codigo","descripcion","familia","stock_actual","consumo_total","meses_con_consumo","ultimo_movimiento","situacion_stock","prioridad","cantidad_a_comprar"]]
            .rename(columns=labels),
            use_container_width=True, hide_index=True, height=min(350,80+len(rot)*35)
        )
    ...
    cols=["codigo","descripcion","familia","unidad_medida","stock_actual","situacion_stock","costo_unitario","valor_inventario","valor_salidas","consumo_total"]+months+[
        ...
        "cantidad_a_comprar","situacion_stock","tipo_consumo","variabilidad","tendencia","anomalia","abc","xyz","prioridad",
