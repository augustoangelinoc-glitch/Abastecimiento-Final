...
with st.sidebar:
    ...
    mostrar_solo_stock_cero=st.checkbox("Mostrar solo stock cero", value=False)
...
if mostrar_solo_stock_cero:view=view[view.stock_actual<=0]
...
with tabs[0]:
    # Los materiales con stock 0 NUNCA se eliminan de la tabla principal.
    # Se mantienen junto con TODOS los análisis y se marca explícitamente
    # su situación de stock.
    cero=df[(df.stock_actual<=0)&(df.consumo_total>0)].copy()
    rot=df[df.situacion_stock=="ROTURA DE STOCK"].copy()
    if len(cero)>0:
        st.warning(
            f"⚠️ STOCK CERO CON SALIDAS: {len(cero):,} materiales | "
            f"🔴 ROTURA DE STOCK: {len(rot):,}"
        )
