...
"stock_actual":"Stock actual","situacion_stock":"Situación de stock","costo_unitario":"Costo unitario (S/)","valor_inventario":"Valor del inventario (S/)",
...
with st.sidebar:
    ...
    only_rotura=st.checkbox("🔴 Solo roturas de stock")
    only_zero=st.checkbox("Stock cero con salidas")
...
if only_rotura:view=view[view.situacion_stock=="ROTURA DE STOCK"]
if only_zero:view=view[(view.stock_actual<=0)&(view.consumo_total>0)]
...
with tabs[0]:
    cero=df[(df.stock_actual<=0)&(df.consumo_total>0)].copy()
    rot=df[df.situacion_stock=="ROTURA DE STOCK"].copy()
    if len(cero)>0:
        st.warning(
            f"⚠️ {len(cero):,} materiales tienen STOCK 0 y registran salidas. "
            f"De ellos, {len(rot):,} están clasificados como ROTURA DE STOCK."
        )
        st.subheader("Materiales con stock cero y consumo")
        ccols=["codigo","descripcion","familia","unidad_medida","stock_actual",
               "consumo_total","meses_con_consumo","meses_sin_consumo",
               "consumo_mensual_prom","consumo_diario","ultimo_movimiento",
               "situacion_stock","prioridad","cantidad_a_comprar","diagnostico"]
        cz=cero[[c for c in ccols if c in cero.columns]].copy()
        if "ultimo_movimiento" in cz.columns:
            cz["ultimo_movimiento"]=pd.to_datetime(cz["ultimo_movimiento"],errors="coerce").dt.strftime("%d/%m/%Y").fillna("")
        cz=cz.rename(columns=labels)
        st.dataframe(cz,use_container_width=True,hide_index=True,height=420)
    st.subheader("Tabla principal de abastecimiento")
    cols=["codigo","descripcion","familia","unidad_medida","stock_actual","situacion_stock","costo_unitario","valor_inventario","valor_salidas","consumo_total"]+months+[
