...
def analyze(stock,sal,oc,horizon=1,z=1.65):
    ...
    ok["lead_time_calc"]=(ok["fecha_guia"]-ok["f_docum"]).dt.days; ok=ok[(ok["lead_time_calc"]>=0)&(ok["lead_time_calc"]<=365)]
    lt=ok.groupby("codigo")["lead_time_calc"].median().rename("lead_time") if not ok.empty else pd.Series(dtype=float)
    ...
    mon["periodo_str"]=mon["periodo"].astype(str); return d,mon,periods
