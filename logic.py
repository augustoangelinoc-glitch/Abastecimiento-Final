...
def load_stock(file):
    ...
    out = pd.DataFrame()
    out["codigo"] = d[code].astype(str).str.strip()
    out["stock_actual"] = pd.to_numeric(d[stock], errors="coerce").fillna(0)
    out["costo_unitario"] = pd.to_numeric(d[cost], errors="coerce").fillna(0)

    desc = find_col(d, ["Descripción", "Descripcion"])
    unidad = find_col(d, ["Unidad de medida", "Unidad", "U.M."])
    familia = find_col(d, ["Familia"])
    out["descripcion"] = d[desc].astype(str) if desc else ""
    out["unidad"] = d[unidad].astype(str) if unidad else ""
    out["familia"] = d[familia].astype(str) if familia else ""
...
def load_salidas(file):
    ...
    out = pd.DataFrame()
    out["codigo"] = d[code].astype(str).str.strip()
    out["fecha"] = pd.to_datetime(d[fecha], errors="coerce", dayfirst=True)
    out["cantidad"] = pd.to_numeric(d[cantidad], errors="coerce").fillna(0)
    out["valor_salida"] = pd.to_numeric(d[valor], errors="coerce") if valor else np.nan
...
def load_oc(file):
    ...
    out["estado_item"] = d[estado].astype(str).str.upper().str.strip() if estado else "COMPRADO"
    return out
