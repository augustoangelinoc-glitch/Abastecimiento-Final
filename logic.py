...
def _find_col(df, aliases, required=False):
    ...
    # 2) coincidencia por tokens (robusta con _, ., -, espacios y tildes)
    for alias in aliases:
        ...
        for c, nc in norm_cols.items():
            ct = set(nc.split())
            if tokens.issubset(ct) or ct.issubset(tokens):
                return c

    # 3) coincidencia parcial, pero evitando que "cantidad" capture una columna
    # descriptiva cuando existe una columna claramente llamada stock.
    for alias in aliases:
        na = _normalize(alias)
        if not na:
            continue
    ...
    if required:
        disponibles = ", ".join(str(c) for c in cols)
        raise ValueError(
            "No se encontró una columna requerida. Alias buscados: "
            f"{aliases}.\n\nColumnas detectadas en el Excel: {disponibles}"
        )
    return None
...
def _read_excel(file):
    """Lee Excel y tolera filas de título antes de los encabezados."""
    try:
        file.seek(0)
    except Exception:
        pass
    ...
    def score(df):
        ...
        groups = [
            ["codigo", "cod", "sku", "item", "material"],
            ["sistema", "stock actual", "stock", "existencia", "saldo", "cantidad"]
        ]
        return sum(
            any(a == c or a in c or c in a for a in group for c in cols)
            for group in groups
        )
    ...
    for header in range(0, 12):
        try:
            file.seek(0)
            d = _clean_headers(pd.read_excel(file, header=header))
            sc = score(d)
            if sc > best_score:
    ...
    if best is None:
        file.seek(0)
        best = _clean_headers(pd.read_excel(file))
    return best
...
STOCK_ALIASES = {
    "codigo": [
        "Código", "Codigo", "codigo", "Cod", "SKU", "Item",
        "Código producto", "Codigo producto", "Cod producto", "ID", "Material"
    ],
    "descripcion": [
        "Descripción", "Descripcion", "desc", "Nombre", "Producto",
        "Nombre producto", "Detalle", "Descripción base", "Descripcion base"
    ],
    "unidad": [
        "Unidad de medida", "U.M.", "U.M", "UM", "Unidad",
        "Unid", "Medida", "U Medida", "U.Medida"
    ],
    "familia": [
        "Familia", "Categoría", "Categoria", "Grupo", "Línea",
        "Linea", "Clase"
    ],
    "stock": [
        "Sistema", "Stock actual", "Stock Actual", "stock_actual", "Stock",
        "Existencia", "Existencias", "Saldo", "Cantidad",
        "Cantidad actual", "Saldo actual", "Stock disponible",
        "Existencia actual", "Disponible"
    ],
    "costo": [
        "Costo unitario", "Costo Unitario (S/)", "Costo", "Costo cierre mes",
        "Costo Cierre Mes", "Costo unitario (S/)", "V/U", "Precio"
    ],
}
