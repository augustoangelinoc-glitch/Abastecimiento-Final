...
def clean_id(s):
    # Identificadores ERP: conservar ceros iniciales y evitar que Excel
    # convierta 000000000003367 en 3367.
    def f(v):
        if pd.isna(v): return ""
        x=str(v).strip()
...
def calculate(stock, salidas, oc=None, ingresos=None):
    ...
    for c in stock.columns:
        if norm(c) in ("material","codigomaterial","codmaterial","codigo"):
            code_s=c
            break
    ...
    s=s[s.codigo!=""].copy()
    s["codigo"]=s["codigo"].str.replace(r"\\s+","",regex=True)
    b["codigo"]=b["codigo"].str.replace(r"\\s+","",regex=True)
    ...
    # Recuperación segura de cruces: si Excel eliminó ceros iniciales en
    # Salidas, se permite un cruce numérico como respaldo, conservando
    # SIEMPRE el código original de Stock Actual para mostrarlo.
    falt=o["consumo_total"].isna()
    if falt.any() and not agg.empty:
        mapa={}
        for _,rr in agg.iterrows():
            k=numeric_key(rr["codigo"])
            if k and k not in mapa:
                mapa[k]=rr
        for i in o.index[falt]:
            rr=mapa.get(numeric_key(o.at[i,"codigo"]))
            if rr is not None:
                o.at[i,"consumo_total"]=rr["consumo_total"]
                o.at[i,"valor_salidas"]=rr["valor_salidas"]
                o.at[i,"ultimo_movimiento"]=rr["ultimo_movimiento"]

    # Recuperación segura de cruces de códigos: si Excel quitó ceros iniciales
    # en Salidas, se permite el cruce por valor numérico SOLO como respaldo.
    # Se conserva como código mostrado el código original de Stock Actual.
    falt=o["consumo_total"].isna()
    if falt.any() and not agg.empty:
        def numkey(x):
            x=str(x).strip()
            return str(int(x)) if re.fullmatch(r"\\d+",x) else x
        mapa={}
        for _,rr in agg.iterrows():
            k=numkey(rr["codigo"])
            if k and k not in mapa: mapa[k]=rr
        for i in o.index[falt]:
            k=numkey(o.at[i,"codigo"])
            rr=mapa.get(k)
            if rr is not None:
                o.at[i,"consumo_total"]=rr["consumo_total"]
                o.at[i,"valor_salidas"]=rr["valor_salidas"]
    ...
    # Se calcula después de clasificar el consumo y de obtener Lead Time.
    o["situacion_stock"]=""
    ...
    def situacion_stock_final(r):
        if r.stock_actual <= 0:
            if r.consumo_total <= 0:
                return "SIN STOCK – SIN CONSUMO"
            if r.meses_con_consumo >= 2 and (
                r.tipo_consumo=="Frecuente"
                or (pd.notna(r.dias_sin_movimiento) and r.dias_sin_movimiento<=60)
            ):
                return "ROTURA DE STOCK"
            if r.meses_con_consumo >= 2:
                return "SIN STOCK – VARIAS SALIDAS"
            return "SIN STOCK – CONSUMO EVENTUAL"
        if pd.notna(r.cobertura_dias) and pd.notna(r.lead_time_dias) and r.lead_time_dias>0 and r.cobertura_dias<r.lead_time_dias:
            return "RIESGO DE ROTURA"
        return "STOCK SUFICIENTE"
    ...
    o["rotura_stock"]=(o["situacion_stock"]=="ROTURA DE STOCK")\n    o["rotura_stock"]=(o["situacion_stock"]=="ROTURA DE STOCK")
