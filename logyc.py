...
def clean_id(s):
    # Identificadores ERP: conservar ceros iniciales.
    def f(v):
        if pd.isna(v):
            return ""
        x=str(v).strip()
...
def calculate(stock, salidas, oc=None, ingresos=None):
    ...
    # Si Excel eliminó ceros iniciales en Salidas, recuperar el cruce por
    # valor numérico como respaldo, conservando el código de Stock.
    ...
    def rec(r):
        ...
        if r.situacion_stock=="SIN STOCK – VARIAS SALIDAS":
            return f"Stock 0.00 con {r.meses_con_consumo} meses con consumo. Revisar abastecimiento."
        if r.situacion_stock=="SIN STOCK – CONSUMO EVENTUAL":
            return "Stock 0.00 con consumo eventual. Validar necesidad antes de comprar."
